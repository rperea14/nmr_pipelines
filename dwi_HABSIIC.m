...
%             end           
            ...
            if isempty(dir_wfp([obj.root '01_GNC/gnc_dMRI*AP.nii']))
                ...
                fn = dir_wfp([obj.root 'Orig/dMRI*AP*.nii*']);
                obj.Params.GradNonlinCorrect.in.fn = fn;
                
                mkdir([obj.root '01_GNC/AP_dwi_grads']);
                
                [m, h] = read_image([fn{1} ',1'],0);
                h = h(1);
                h.fname = [obj.root '01_GNC/AP_dwi_grads/firstb0.nii'];
                spm_write_vol(h,m);
                ...
                system(['mv ' obj.root 'Orig/gnc* ' obj.root '01_GNC/']);
...
%                 system(['mv ' rp 'CommonProc/01_GNC/AP_se_grads/gnc_0*AP.nii ' rp 'CommonProc/01_GNC/']);
            ...
            %%
             if isempty(dir_wfp([obj.root '01_GNC/gnc_dMRI*PA.nii']))
            
                obj.Params.GradNonlinCorrect.in.movefiles = '';
                obj.Params.GradNonlinCorrect.in.prefix = 'gnc_';
                obj.Params.GradNonlinCorrect.in.gradfile = '/autofs/cluster/hab/HAB/HABS_IIC/Scripts/coeff_AS82.grad';
                fn = dir_wfp([obj.root 'Orig/dMRI*PA*.nii*']);
                obj.Params.GradNonlinCorrect.in.fn = fn;
                
                mkdir([obj.root '01_GNC/PA_dwi_grads']);
                
                [m, h] = read_image([fn{1} ',1'],0);
                h = h(1);
                h.fname = [obj.root '01_GNC/PA_dwi_grads/firstb0.nii'];
                spm_write_vol(h,m);
...
%                 system(['mv ' rp 'CommonProc/01_GNC/AP_se_grads/gnc_0*AP.nii ' rp 'CommonProc/01_GNC/']);
            ...
            obj.Params.B0MoCo.in.movefiles = '../02_B0s_MoCo/';
            obj.Params.B0MoCo.in.fn = dir_wfp([obj.root '01_GNC/gnc_dMRI*.nii']);
            bvals = []; bvecs = [];
            for ii = 1:numel(obj.Params.B0MoCo.in.fn)
                [s1 s2 s3] = fileparts(obj.Params.B0MoCo.in.fn{ii});
                tmp = dir_wfp([obj.root 'Orig/' regexprep(s2,'gnc_','') '.bval']);
                bvals{ii,1} = tmp{1};
                tmp = dir_wfp([obj.root 'Orig/' regexprep(s2,'gnc_','') '.bvec']);
                bvecs{ii,1} = tmp{1};
            end
            obj.Params.B0MoCo.in.bvals = bvals;
            obj.Params.B0MoCo.in.bvecs = bvecs;
